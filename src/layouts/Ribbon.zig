...
pub const Ribbon = struct {
    pub fn arrange(self: *Ribbon, ws: *Workspace, box: wlr.Box) void {
        ...
        while (it != &ws.views.link) : (it = it.?.prev) {
            ...
            // Floating windows, fullscreen windows, maximized windows, or the currently grabbed window stay at their own x/y
            if (view.is_floating or view.is_fullscreen or view.is_maximized or view.server.grabbed_view == view) {
